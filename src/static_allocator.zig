...
const State = enum {
    /// Allow `alloc`, `resize`, and `remap`.
    /// `free` is also allowed so that errdefer cleanup is easy to write;
    /// after the first `free` the state switches to `.deinit`.
    init,
    /// Don't allow any calls.
    static,
    /// Allow `free` but not `alloc`, `resize`, or `remap`.
    deinit,
};
